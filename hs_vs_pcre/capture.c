...
int main(int argc, char *argv[])
{
    ...
    unsigned int m;
    int s_match;
#define MAX_CAPTURE_CNT 16
    int ovector[MAX_CAPTURE_CNT] = {0};
    ...
    for (i = 0; i <= grep_c; i++) {
        ...
        printf("text data: %s\n", p);
        if ('p' == *type) {
            memset(ovector, 0, sizeof(int) * MAX_CAPTURE_CNT);
            s_match = pcre_exec(p_pcre, NULL, p, strlen(p), 0, 0, ovector, MAX_CAPTURE_CNT);
            if (s_match < 0) {
                //printf("%s doesn't match %s\n", pText, pattern);
            } else {
                g_match++;
                printf("-----------matches %s , times %d \n", pattern, s_match);
                for (m = 0; m < s_match; m++) {
                    if (m * 2 >= MAX_CAPTURE_CNT) {
                        break;
                    }
                    printf("%d, %d ", ovector[2 * m], ovector[2 * m + 1]);
                }
            }
            printf("\n");
        } else {
    ...
    printf("global match line counter: %d.\n", g_match);
