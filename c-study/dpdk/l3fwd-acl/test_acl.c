...
static void
acl_nta_config(struct rte_acl_config *cfg, uint32_t num_categories)
{
	static const struct rte_acl_field_def
		ipv4_defs[RTE_ACL_IPV4VLAN_NUM_FIELDS] = {
		{
			...
			.field_index = RTE_ACL_IPV4VLAN_PROTO_FIELD,
			.input_index = RTE_ACL_IPV4VLAN_PROTO,
            .offset = offsetof(struct ipv4_7tuple, proto),
		},
		{
			.type = RTE_ACL_FIELD_TYPE_BITMASK,
			.size = sizeof(uint16_t),
			.field_index = RTE_ACL_IPV4VLAN_VLAN1_FIELD,
			.input_index = RTE_ACL_IPV4VLAN_VLAN,
            .offset = offsetof(struct ipv4_7tuple, vlan),
		},
		{
			.type = RTE_ACL_FIELD_TYPE_BITMASK,
			.size = sizeof(uint16_t),
			.field_index = RTE_ACL_IPV4VLAN_VLAN2_FIELD,
			.input_index = RTE_ACL_IPV4VLAN_VLAN,
            .offset = offsetof(struct ipv4_7tuple, domain),
		},
		{
			.type = RTE_ACL_FIELD_TYPE_MASK,
			.size = sizeof(uint32_t),
			.field_index = RTE_ACL_IPV4VLAN_SRC_FIELD,
			.input_index = RTE_ACL_IPV4VLAN_SRC,
            .offset = offsetof(struct ipv4_7tuple, ip_src),
		},
		{
			.type = RTE_ACL_FIELD_TYPE_MASK,
			.size = sizeof(uint32_t),
			.field_index = RTE_ACL_IPV4VLAN_DST_FIELD,
			.input_index = RTE_ACL_IPV4VLAN_DST,
            .offset = offsetof(struct ipv4_7tuple, ip_dst),
		},
		{
			.type = RTE_ACL_FIELD_TYPE_RANGE,
			.size = sizeof(uint16_t),
			.field_index = RTE_ACL_IPV4VLAN_SRCP_FIELD,
			.input_index = RTE_ACL_IPV4VLAN_PORTS,
            .offset = offsetof(struct ipv4_7tuple, port_src),
		},
		{
			.type = RTE_ACL_FIELD_TYPE_RANGE,
			.size = sizeof(uint16_t),
			.field_index = RTE_ACL_IPV4VLAN_DSTP_FIELD,
			.input_index = RTE_ACL_IPV4VLAN_PORTS,
            .offset = offsetof(struct ipv4_7tuple, port_dst),
		},
	};
...
struct rte_acl_ipv4vlan_rule nta_rules[] = {
...
/* source IP addresses */
		...
		{
				.data = {.userdata = 5, .category_mask = ACL_DENY_MASK,
						.priority = 40},
				.src_addr = IPv4(11,1,1,0),
				.src_mask_len = 24,
				.src_port_low = 0,
				.src_port_high = 0xffff,
				.dst_port_low = 0,
		...
		{
				.data = {.userdata = 6, .category_mask = 0x80000000,
						.priority = 40},
				.src_addr = IPv4(11,1,0,0),
				.src_mask_len = 16,
				.src_port_low = 0,
				.src_port_high = 0xffff,
				.dst_port_low = 0,
...
static int test_nta(void)
{
	static struct ipv4_7tuple nta_data[] = {
		{
			.proto = 6,
			.ip_src = IPv4(10, 1, 1, 1),
			.ip_dst = IPv4(192, 168, 0, 33),
	...
	struct rte_acl_ctx *acx;
	struct rte_acl_config cfg;
	uint32_t results[RTE_DIM(nta_data) * RTE_ACL_MAX_CATEGORIES];
	const uint8_t *data[RTE_DIM(nta_data)];
    ...
    ret = rte_acl_ipv4vlan_add_rules(acx, nta_rules, RTE_DIM(nta_rules));
    if (ret != 0) {
        printf("Line %i: Adding rules to ACL context failed!\n", __LINE__);
        return ret;
    }
	...
	memset(&cfg, 0, sizeof(cfg));
	acl_nta_config(&cfg, RTE_ACL_MAX_CATEGORIES);
	...
	/* swap all bytes in the data to network order */
	bswap_test_data(nta_data, RTE_DIM(nta_data), 1);

	/* store pointers to test data */
	for (i = 0; i != RTE_DIM(nta_data); i++)
		data[i] = (uint8_t *)&nta_data[i];

    ret = rte_acl_classify(acx, data, results, RTE_DIM(data), RTE_ACL_MAX_CATEGORIES);
    if (ret != 0) {
        printf("iter: %d: classify failed!\n", __LINE__);
    }
	for (i = 0; i != RTE_DIM(results); i++) {
        if (0 == results[i]) {
            continue;
        }
        printf("results[%2d] is: %d.\n", i, results[i]);
	}

	bswap_test_data(nta_data, RTE_DIM(nta_data), 0);
	rte_acl_free(acx);
...
#endif
    if (test_nta() < 0) {
        printf("xxxxxxxxxxxxxxxxx test failed!\n");
        return -1;
    } else {
        printf("acl nta test success!\n");
    }
#if 0
